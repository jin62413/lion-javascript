...
function first() {
  ...
  function second() {
    ...
    return x + y; // 상위 컨텍스트에 있는 x를 찾아서 적용
  }

  // return second(); // 15
  return second; // 함수를 실행하지 않고 본문을 내보냄
}
// first(); // 15

// currying function
// first()(); // first 실행 -> second 실행, first(1)() first로 감, first()(1) second로 감
const value = first() // 호출: value() -> 15출력
/* 
value() === first()(), 변수를 함수처럼 사용하는 것
first()는 second 본문을 내뱉기 때문에 value()를 하면 second()를 한 것과 같은 효과를 가짐
함수를 실행한 게 아니고 함수 본문을 내뱉은 것!
함수는 본인이 태어난 곳을 기억하고 있다!! (클로저의 개념)
함수 바깥에서 second를 호출해서 원래 사용자는 x에 접근할 수 없다.
하지만 second는 first안에 있는 본인을 기억 -> x에 접근할 수 있다.
value에 함수 본문 전체를 등록해놨기 때문에 메모리에 계속 남아있다.
계속 참조가 가능하기 때문에 가비지 컬렉터가 수집하지 않음.
가비지 컬렉터가 들어오지 못하도록 폐쇄 = closure
 */
